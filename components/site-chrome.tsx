...
export default function SiteChrome({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const refresh = (event?: Event) => {
      ...
      if (detail) { setCount(detail.totalQuantity); return; }
  ...
  return <main className="min-h-screen bg-paper text-ink">
    {toast && <div className={`cart-toast ${toast.kind === "error" ? "error" : ""}`} role={toast.kind === "error" ? "alert" : "status"} aria-live={toast.kind === "error" ? "assertive" : "polite"}><span>{toast.kind === "error" ? "!" : "✓"}</span><strong>{toast.message}</strong><button type="button" onClick={() => setToast(null)} aria-label="關閉通知">×</button></div>}
    ...
    <header className="site-header">
      <Link href="/" className="logo" aria-label="敲敲韓國代購首頁">敲敲<span>韓國代購</span><i></i></Link>
      <nav aria-label="主要導覽"><Link href="/">首頁</Link><Link href="/products">全部商品</Link></nav>
      <div className="header-actions">
        <form ref={searchRef} className={`header-search ${searchOpen ? "open" : ""}`} role="search" onSubmit={submitSearch} onKeyDown={(event) => { if (event.key === "Escape") { setSearchOpen(false); (event.currentTarget.querySelector("button") as HTMLButtonElement | null)?.focus(); } }}><label className="sr-only" htmlFor="header-product-search">搜尋商品</label><FormInput ref={searchInputRef} id="header-product-search" type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="搜尋商品名稱" autoComplete="off" tabIndex={searchOpen ? 0 : -1} /><button className="circle-button header-search-button" type="submit" aria-label={searchOpen ? "送出商品搜尋" : "展開商品搜尋"} aria-expanded={searchOpen} aria-controls="header-product-search">⌕</button></form>
        <div className={`account-menu ${accountOpen ? "open" : ""}`} ref={accountRef}>
          ...
          >
            <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.5" /><path d="M5 20c.5-4 3-6 7-6s6.5 2 7 6" /></svg>
          </button>
          {accountOpen && (
            <div className="account-dropdown" role="menu">
              {hasHydrated && signedIn ? (
                <>
                  ...
                  <button type="button" role="menuitem" onClick={() => { logoutUser(); setCount(0); setAccountOpen(false); }}>登出</button>
                </>
              ) : (
        ...
        <Link className="cart-button cart-icon-button" href="/cart" aria-label={`購物車，共 ${count} 件商品`}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 4h2l2.2 10.1a2 2 0 0 0 2 1.6h7.9a2 2 0 0 0 1.9-1.4L21 8H6.1M10 20a1 1 0 1 1-2 0 1 1 0 0 1 2 0Zm9 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0Z" /></svg><span>{count}</span></Link>
      </div>
    </header>
    {children}
    <footer><Link href="/" className="logo footer-logo">敲敲韓國代購</Link><p>MAKE YOUR FANDOM SHINE.</p><div><Link href="/products">商品總覽</Link><Link href="/products?sort=newest">最新商品</Link><a href="mailto:hello@knock-knock.tw">聯絡我們</a><a href="https://instagram.com">Instagram</a></div><small>© 2026 KNOCK-KNOCK. ALL RIGHTS RESERVED.</small></footer>
  </main>;
}
